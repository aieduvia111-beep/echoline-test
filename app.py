from flask import Flask, request, Response, session, redirect, url_for, render_template_string, send_from_directory
import requests
import os
import json
import time
...
@app.route("/prezentacja")
def prezentacja():
    """Publicznie dostepna prezentacja PDF - link z kodu QR na wizytowce."""
    base_dir = os.path.dirname(os.path.abspath(__file__))
    return send_from_directory(base_dir, "EchoLine_prezentacja.pdf")
...
<script>
  ...
  window.editField = function(name){
    ...
    const valueEl = document.querySelector(`[data-field="${name}"]`);
    const saveBtnEl = document.querySelector(`[data-savebtn="${name}"]`);
    const isCurrentlyEditing = inputEl.style.display === 'block';

    if (isCurrentlyEditing) {
      // Drugie klikniecie olowka - anuluj edycje, przywroc ostatnio ZAPISANA wartosc (nie zapisuj tego co wpisano)
      inputEl.value = inputEl.dataset.savedValue || '';
      inputEl.style.display = 'none';
      valueEl.style.display = 'block';
      saveBtnEl.style.display = 'none';
    } else {
      if (inputEl.dataset.savedValue === undefined) inputEl.dataset.savedValue = inputEl.value;
...
<script type="module">
  ...
  window.saveField = async function(name){
    ...
    inputEl.dataset.savedValue = value;
    const displayEl = document.querySelector(`[data-field="${name}"]`);
