...
<script type="module">
  ...
  const I18N = {
    pl: {
      ...
      err_fill_email_for_reset: "Wpisz email powyżej, potem kliknij \"Zapomniałem hasła\".",
...
<div class="main">
  ...
  <div id="sufBox" style="display:none;position:fixed;bottom:24px;right:24px;left:auto;max-width:360px;background:#fff;border:1.5px solid #7c6aff;border-radius:16px;padding:18px;box-shadow:0 20px 50px -15px rgba(124,106,255,0.4);z-index:999;">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
      ...
      <b style="font-size:13px;">Bot potrzebuje podpowiedzi</b>
    </div>
    <p id="sufQuestion" style="font-size:13px;color:#555;margin-bottom:12px;"></p>
    <div style="display:flex;gap:8px;">
      <input id="sufInput" type="text" placeholder="Wpisz co ma powiedzieć..." style="flex:1;padding:9px 12px;border-radius:9px;border:1px solid #ddd;font-size:13px;font-family:inherit;">
      <button onclick="sendHint()" style="padding:9px 14px;border-radius:9px;border:none;background:linear-gradient(135deg,#7c6aff,#5dadff);color:#fff;font-weight:700;font-size:13px;cursor:pointer;">Wyślij</button>
    </div>
  </div>
...
<script type="module">
  ...
  window.choosePlan = function(plan){
    ...
    if (!currentUid) {
      msg.style.display = 'block';
      msg.textContent = 'Musisz byc zalogowany, zeby wykupic plan.';
      return;
    }
    msg.style.display = 'block';
  ...
  if (params.get('payment') === 'success') {
    const msg = document.getElementById('actionMsg');
    msg.style.display = 'block';
    msg.textContent = 'Platnosc przyjeta! Aktywujemy Twoj plan (to zwykle zajmuje kilka sekund)...';
    ...
    onAuthStateChanged(auth, (user) => {
      ...
      onSnapshot(doc(db, "users", user.uid), (snap) => {
        ...
        if (data.subscriptionActive) {
          ...
          msg.textContent = 'Plan (' + (data.plan || '') + ') zostal aktywowany! Mozesz teraz korzystac z EchoLine.';
        }
      });
    });
    ...
    <div id="usageSection" style="margin-top:18px;">
      <div style="margin-bottom:14px;">
        <div style="display:flex;justify-content:space-between;font-size:12.5px;margin-bottom:6px;">
          <span style="color:#666;font-weight:600;">Minuty rozmów</span>
      ...
      <div>
        <div style="display:flex;justify-content:space-between;font-size:12.5px;margin-bottom:6px;">
          <span style="color:#666;font-weight:600;">Zlecenia "Zadzwoń za mnie"</span>
...
<script type="module">
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    // Wypelnij paski zuzycia (minuty i zlecenia) prawdziwymi danymi z bazy
    ...
    document.getElementById("minutesText").textContent = `${minutesLeft} min zostało (${minutesUsed} / ${limits.minutes})`;
    document.getElementById("minutesBar").style.width = limits.minutes > 0 ? Math.min((minutesUsed / limits.minutes) * 100, 100) + "%" : "0%";

    document.getElementById("quickCallsText").textContent = `${quickCallsLeft} zostało (${quickCallsUsed} / ${limits.quickCalls})`;
    ...
    // Pokaz kiedy nastapi kolejny reset (30 dni od aktywacji planu)
    ...
    if (data.planActivatedAt) {
      ...
      resetInfoEl.textContent = "Orientacyjny termin odnowienia: " + nextReset.toLocaleDateString("pl-PL") + " (dokladna data zalezy od Twojego cyklu rozliczeniowego w Stripe)";
    } else {
...
def limit_exceeded_twiml(reason):
    return f'''<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Say language="pl-PL">Dzien dobry, przepraszam, ale w tej chwili nie moge przyjac polaczenia. Prosze sprobowac zadzwonic pozniej, lub napisac wiadomosc. Dziekuje.</Say>
    <Hangup/>
</Response>'''
...
SUFLER_INSTRUCTION = (
    ...
    "formacie: POTRZEBUJE_POMOCY: [krotki opis o co pyta rozmowca]"
)
...
def build_profile_prompt(user_data):
    ...
    return (
        ...
        f"Rozmawiaj po polsku, krotko, uprzejmie i konkretnie. "
...
def incoming_call():
    ...
    if owner_uid and owner_data:
        ...
        if not ok:
            return Response(limit_exceeded_twiml("minutes"), mimetype="text/xml")
    ...
    if company_name:
        greeting = f"Dzien dobry, tu automatyczny asystent AI dzialajacy w imieniu {company_name}. Rozmowa moze byc transkrybowana. Slucham, w czym moge pomoc?"
    else:
        greeting = "Dzien dobry, tu automatyczny asystent AI. Rozmowa moze byc transkrybowana. Slucham, w czym moge pomoc?"
...
<Response>
    <Say language="pl-PL">Dziekuje bardzo za rozmowe. To wszystkie informacje jakie mam w tej chwili - w razie dodatkowych pytan prosze o kontakt bezposredni. Milego dnia, do uslyszenia.</Say>
    <Hangup/>
</Response>''',
                mimetype="text/xml"
            )
    ...
    if reply.strip().startswith("POTRZEBUJE_POMOCY:") and ctx.get("owner_uid"):
        ...
        stall_text = "Chwileczke, sprawdzam to dla Pana/Pani."
...
def check_hint():
    ...
    if attempt >= 6:
        ...
        fallback_text = "Przepraszam za oczekiwanie. Postaram sie to sprawdzic i oddzwonic w tej sprawie. Czy moge pomoc w czyms jeszcze?"
