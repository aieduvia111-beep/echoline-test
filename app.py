...
def get_plan_limits(user_data):
    plan = (user_data or {}).get("plan", "free")
    return PLAN_LIMITS.get(plan, PLAN_LIMITS["free"]), plan


def maybe_reset_usage(uid, user_data):
    """
    Zeruje liczniki (minutesUsed, quickCallsUsed) jesli minelo 30+ dni
    od ostatniego resetu / aktywacji planu - reset nastepuje wiec zawsze
    w "rocznice" zakupu (np. kupione 5 czerwca -> reset 5 lipca, 5 sierpnia...),
    a nie sztywno 1. dnia kazdego miesiaca dla wszystkich naraz.
    Zwraca zaktualizowane user_data (do dalszego uzycia w tym samym requescie).
    """
    if not db_admin or not uid or not user_data:
        return user_data

    activated_at = user_data.get("planActivatedAt")
    if not activated_at:
        return user_data

    try:
        # planActivatedAt zapisywane jest jako ISO string (patrz PRICING_PAGE / dashboard)
        activated_dt = datetime.fromisoformat(activated_at)
    except Exception:
        return user_data

    now = datetime.utcnow()
    days_since = (now - activated_dt).days
    ...
    if days_since >= 30:
        ...
        ref.set({
            "minutesUsed": 0,
            "quickCallsUsed": 0,
            "planActivatedAt": now.isoformat(),
        }, merge=True)
        user_data["minutesUsed"] = 0
        user_data["quickCallsUsed"] = 0
        user_data["planActivatedAt"] = now.isoformat()
...
<script type="module">
  ...
  if (params.get('payment') === 'success') {
    ...
    onAuthStateChanged(auth, async (user) => {
      if (!user) return;
      await updateDoc(doc(db, "users", user.uid), { plan: plan, planActivatedAt: new Date().toISOString(), minutesUsed: 0, quickCallsUsed: 0 });
      const msg = document.getElementById('actionMsg');
      msg.style.display = 'block';
...
def incoming_call():
    ...
    if owner_uid and owner_data:
        owner_data = maybe_reset_usage(owner_uid, owner_data)
...
def call_status():
    """Twilio wywoluje ten endpoint gdy polaczenie sie konczy - zapisujemy zuzyte minuty."""
    call_sid = request.form.get("CallSid", "unknown")
...
def start_outbound_call():
    ...
    user_data = maybe_reset_usage(uid, user_data)
