...
def get_user_calls(uid, limit=50, offset=0):
...
<script>
  ...
  function updateCallsCounter(){
    ...
    if (SEARCH_CACHE && searchActive) {
      ...
      counterEl.textContent = 'Znaleziono: ' + visibleCount;
    } else {
  ...
  let callsOffset = ALL_CALLS.length;
  ...
  async function loadMoreCalls(){
    ...
    try {
      const resp = await fetch('/api/more-calls?offset=' + callsOffset);
      const result = await resp.json();
...
def api_more_calls():
    ...
    more_calls = get_user_calls(session.get("uid", ""), limit=20, offset=offset)
    return {"calls": more_calls, "has_more": len(more_calls) == 20}

@app.route("/api/search-calls")
def api_search_calls():
    """Pobiera WSZYSTKIE rozmowy uzytkownika naraz - uzywane tylko podczas wyszukiwania."""
    if not session.get("logged_in"):
        return {"error": "not logged in"}, 401
    all_calls = get_user_calls(session.get("uid", ""), limit=1000)
