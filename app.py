...
# --- Keep-alive: Render (darmowy plan) usypia serwer po ~15 min braku ruchu ---
# Ten watek w tle sam odwiedza strone co 10 minut, zeby nie doszlo do usniecia.
# UWAGA: to dziala tylko gdy serwer JUZ nie spi - jesli faktycznie zasnie
# (np. restart serwera), pierwsze zapytanie od kogokolwiek i tak go obudzi,
# tylko z kilkusekundowym opoznieniem. Dla stuprocentowej pewnosci najlepiej
# dodatkowo ustawic zewnetrzny serwis (np. darmowy UptimeRobot.com) pingujacy
# adres /health co 10-14 minut - to bardziej niezawodne niz sam-w-sobie ping.
APP_URL = os.environ.get("APP_URL", "https://echoline-test.onrender.com")
...
def keep_alive_loop():
    while True:
        ...
        try:
            ...
            print("[keep_alive] blad pingowania:", e)
...
BOT_LANGUAGES = {
    "pl": {
        ...
        "outbound_greeting_prefix": "Dzien dobry, dzwonie w imieniu mojego uzytkownika w nastepujacej sprawie: ",
    },
    "en": {
        "twilio": "en-US",
        ...
        "outbound_greeting_prefix": "Hello, I am calling on behalf of my user regarding the following matter: ",
    },
    "de": {
        "twilio": "de-DE",
        ...
        "outbound_greeting_prefix": "Guten Tag, ich rufe im Auftrag meines Nutzers an, bezueglich: ",
    },
}
...
def get_user_calls_count(uid):
    """Zwraca laczna liczbe rozmow danego uzytkownika (do licznika 'pokazano X z Y')."""
    if not db_admin or not uid:
        return 0
    try:
        query = db_admin.collection("calls").where("uid", "==", uid)
        result = query.count().get()
        return result[0][0].value
    except Exception as e:
        print("[get_user_calls_count] blad:", e)
        return 0


def get_user_calls(uid, limit=50, after_created_at=None):
    """
    Pobiera prawdziwe rozmowy danego uzytkownika z bazy, posortowane od najnowszej.
    Uzywa "cursora" (start_after) zamiast offsetu - dzieki temu szybkosc NIE spada
    nawet przy tysiacach rozmow (offset musialby za kazdym razem "przeliczac"
    wszystkie pominiete rekordy, co przy duzej skali robi sie coraz wolniejsze).
    """
    if not db_admin or not uid:
        return []
    try:
        ...
        if after_created_at:
            query = query.start_after({"created_at": after_created_at})
...
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Cdefs%3E%3CradialGradient id='g' cx='30%25' cy='30%25'%3E%3Cstop offset='0%25' stop-color='%23a9c9ff'/%3E%3Cstop offset='60%25' stop-color='%237c6aff'/%3E%3Cstop offset='100%25' stop-color='%235a4bd4'/%3E%3C/radialGradient%3E%3C/defs%3E%3Ccircle cx='16' cy='16' r='14' fill='url(%23g)'/%3E%3C/svg%3E">
...
<div class="main">
  ...
  <div id="testChatModal" onclick="if(event.target===this) closeTestChat();" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:1000;align-items:center;justify-content:center;padding:20px;">
    <div style="background:#fff;border-radius:20px;max-width:460px;width:100%;max-height:82vh;display:flex;flex-direction:column;overflow:hidden;">
      <div style="padding:16px 20px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center;background:linear-gradient(135deg,#f8f7ff,#eef6ff);">
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="width:8px;height:8px;border-radius:50%;background:#22d3a0;"></div>
          <b style="font-size:13.5px;">Testowa rozmowa z botem</b>
        </div>
        <button onclick="closeTestChat()" style="background:#fff;border:none;width:28px;height:28px;border-radius:50%;cursor:pointer;font-size:14px;color:#666;">✕</button>
      </div>
      <div id="testChatMessages" style="padding:18px 20px;overflow-y:auto;flex:1;background:#fafafa;"></div>
      <div style="padding:14px 16px;border-top:1px solid #eee;display:flex;gap:8px;">
        <input id="testChatInput" type="text" placeholder="Napisz jak dzwoniący klient..." onkeydown="if(event.key==='Enter') sendTestChatMessage();" style="flex:1;padding:10px 14px;border-radius:100px;border:1px solid #ddd;font-size:13px;font-family:inherit;">
        <button onclick="sendTestChatMessage()" style="padding:10px 18px;border-radius:100px;border:none;background:linear-gradient(135deg,#7c6aff,#5dadff);color:#fff;font-weight:700;font-size:13px;cursor:pointer;">Wyślij</button>
      </div>
    </div>
  </div>
  ...
  <div class="content">
    ...
    <div id="section-assistant">
      ...
      <div class="grid2 rise4">
        ...
        <div class="card">
          ...
          <button onclick="openTestChat()" style="width:100%;margin-top:16px;padding:12px;border-radius:12px;border:none;background:linear-gradient(135deg,#7c6aff,#5dadff);color:#fff;font-weight:700;font-size:13.5px;cursor:pointer;">
            💬 Testuj rozmowę z botem (bez telefonu)
          </button>
        </div>
      </div>
    ...
    <div id="section-quickcall" style="display:none;">
      <div class="card call-wrap rise3">
        ...
        <label style="font-size:12.5px;font-weight:600;color:#555;display:block;margin-bottom:6px;" data-i18n="qc_lang_label">W jakim języku bot ma rozmawiać?</label>
        <select id="quickCallLang" style="width:100%;padding:12px 10px;border-radius:12px;border:1px solid #ddd;font-family:inherit;font-size:14px;background:#fff;margin-bottom:16px;">
          <option value="pl" data-i18n="lang_opt_pl">Polski</option>
          <option value="en" data-i18n="lang_opt_en">Angielski</option>
          <option value="de" data-i18n="lang_opt_de">Niemiecki</option>
        </select>
...
<script>
  ...
  const DASH_I18N = {
    pl: {
      ...
      qc_lang_label: "W jakim języku bot ma rozmawiać?",
    ...
    en: {
      ...
      qc_lang_label: "What language should the bot speak?",
  ...
  window.quickCall = async function(){
    ...
    const lang = document.getElementById('quickCallLang').value;
    ...
    try {
      const resp = await fetch('/start-outbound-call', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ phone_number: phone, task: task, lang: lang })
      });
...
<script type="module">
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (document.getElementById('quickCallLang')) document.getElementById('quickCallLang').value = data.botLanguage || 'pl';
  ...
  window.openTestChat = function(){
    ...
    document.getElementById('testChatModal').style.display = 'flex';
    appendTestChatBubble('assistant', 'Dzień dobry, tu automatyczny asystent AI. Rozmowa może być transkrybowana. Słucham, w czym mogę pomóc?');
    testChatHistory.push({role: 'assistant', content: 'Dzień dobry, tu automatyczny asystent AI. Rozmowa może być transkrybowana. Słucham, w czym mogę pomóc?'});
    setTimeout(() => document.getElementById('testChatInput').focus(), 100);
  };

  window.closeTestChat = function(){
    document.getElementById('testChatModal').style.display = 'none';
  };

  function appendTestChatBubble(role, text, isSufler){
    const wrap = document.getElementById('testChatMessages');
    const isBot = role === 'assistant';
    const bg = isSufler ? '#fff3cd' : (isBot ? '#f5f3ff' : '#111');
    const color = isSufler ? '#8a6d1f' : (isBot ? '#333' : '#fff');
    const align = isBot ? 'flex-start' : 'flex-end';
    const label = isSufler ? 'Sufler (nie wie co odpowiedzieć)' : (isBot ? 'Bot' : 'Ty (jako klient)');
    const div = document.createElement('div');
    div.style.cssText = 'display:flex;justify-content:' + align + ';margin-bottom:10px;';
    div.innerHTML = '<div style="max-width:82%;background:' + bg + ';color:' + color + ';padding:10px 14px;border-radius:14px;font-size:13.5px;line-height:1.4;">' +
      '<div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.04em;opacity:0.6;margin-bottom:3px;">' + label + '</div>' +
      text + '</div>';
  ...
  window.sendTestChatMessage = async function(){
    ...
    appendTestChatBubble('user', message);
    testChatHistory.push({role: 'user', content: message});

    const wrap = document.getElementById('testChatMessages');
    const thinking = document.createElement('div');
    thinking.id = 'testChatThinking';
    thinking.style.cssText = 'font-size:12px;color:#999;padding:4px 14px;';
    thinking.textContent = 'Bot pisze...';
    ...
    try {
      const resp = await fetch('/api/test-chat', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ message: message, history: testChatHistory })
      });
      const result = await resp.json();
      document.getElementById('testChatThinking').remove();

      if (result.error) {
        appendTestChatBubble('assistant', 'Błąd: ' + result.error);
        return;
      }

      appendTestChatBubble('assistant', result.reply, result.is_sufler);
      testChatHistory.push({role: 'assistant', content: result.reply});
    } catch (e) {
      const t = document.getElementById('testChatThinking');
      if (t) t.remove();
      appendTestChatBubble('assistant', 'Błąd połączenia: ' + e.message);
    }
  };
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .main{flex:1;display:flex;flex-direction:column;min-width:0;}
  .columns-row{display:flex;flex:1;min-width:0;}
...
<div class="main">
  <div class="lang-switch-top" style="display:flex;justify-content:flex-end;gap:4px;margin:16px 32px 0;">
...
<div class="main rise">
  ...
  <div class="filters rise3" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
    ...
    <input id="callsSearchInput" type="text" placeholder="Szukaj po numerze telefonu..." oninput="handleCallsSearch(this.value)" style="padding:8px 14px;border-radius:100px;border:1px solid #eee;font-size:12.5px;font-family:inherit;min-width:200px;">
  </div>

  <div id="callsCounter" style="font-size:12px;color:#999;margin-bottom:10px;"></div>
  ...
  <div class="calls-table rise4">
    <table id="callsTable">
      ...
      {% for call in calls %}
      <tr class="datarow" data-type="incoming" data-status="ok" data-call-index="{{ loop.index0 }}" onclick="showTranscript({{ loop.index0 }})" style="cursor:pointer;">
        <td>{{ call.data }}</td>
    ...
    <div id="callsLoading" style="display:none;text-align:center;padding:16px;font-size:12.5px;color:#999;">Ładowanie kolejnych rozmów...</div>
    <div id="callsSentinel" style="height:1px;"></div>
  </div>
  <script type="application/json" id="transcriptsData">{{ calls | tojson }}</script>

  <button id="scrollTopBtn" onclick="window.scrollTo({top:0,behavior:'smooth'});" style="display:none;position:fixed;bottom:24px;right:24px;width:46px;height:46px;border-radius:50%;background:linear-gradient(135deg,#7c6aff,#5dadff);border:none;box-shadow:0 10px 24px -6px rgba(124,106,255,0.5);cursor:pointer;z-index:900;align-items:center;justify-content:center;">
    <svg viewBox="0 0 24 24" style="width:20px;height:20px;stroke:#fff;fill:none;stroke-width:2.4;"><path d="M18 15l-6-6-6 6"/></svg>
  </button>
  ...
  <div id="transcriptModal" onclick="if(event.target===this) closeTranscript();" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:1000;align-items:center;justify-content:center;padding:20px;">
    <div style="background:#fff;border-radius:20px;max-width:480px;width:100%;max-height:80vh;display:flex;flex-direction:column;overflow:hidden;">
      <div style="padding:18px 22px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center;">
        <div>
          <b id="modalDate" style="font-size:14px;"></b>
          <div id="modalWith" style="font-size:12.5px;color:#999;margin-top:2px;"></div>
        </div>
        <button onclick="closeTranscript()" style="background:#f5f5f5;border:none;width:30px;height:30px;border-radius:50%;cursor:pointer;font-size:16px;color:#666;">✕</button>
      </div>
      <div id="modalBody" style="padding:20px 22px;overflow-y:auto;flex:1;"></div>
    </div>
  </div>
</div>
...
<script>
  ...
  let ALL_CALLS = [];
  try {
    const dataEl = document.getElementById('transcriptsData');
    if (dataEl) ALL_CALLS = JSON.parse(dataEl.textContent);
  } catch (e) { ALL_CALLS = []; }

  const TOTAL_CALLS = {{ total_calls | default(0) }};
  ...
  function updateCallsCounter(){
    ...
    if (!counterEl) return;
    const searchActive = document.getElementById('callsSearchInput') && document.getElementById('callsSearchInput').value.trim();
    if (SEARCH_CACHE && searchActive) {
      let visibleCount = 0;
      document.querySelectorAll('#callsTable .datarow').forEach(function(row){
        if (row.style.display !== 'none') visibleCount++;
      });
      let text = 'Znaleziono: ' + visibleCount;
      if (searchTruncated) {
        text = '<span style="color:#c9840f;">⚠ Masz ponad ' + searchTotal + ' rozmów - przeszukano tylko ' + SEARCH_CACHE.length + ' najnowszych. ' + text + '</span>';
        counterEl.innerHTML = text;
      } else {
        counterEl.textContent = text;
      }
    } else {
      counterEl.textContent = 'Pokazano ' + ALL_CALLS.length + ' z ' + TOTAL_CALLS + ' rozmów';
    }
  }
  updateCallsCounter();

  window.showTranscript = function(index){
    const call = ALL_CALLS[index];
    if (!call) return;

    document.getElementById('modalDate').textContent = call.data;
    document.getElementById('modalWith').textContent = call.z_kim;

    const body = document.getElementById('modalBody');
    const transcript = call.transcript || [];

    if (transcript.length === 0) {
      body.innerHTML = '<p style="font-size:13px;color:#999;">Brak zapisanej transkrypcji dla tej rozmowy.</p>';
    } else {
      body.innerHTML = transcript.map(function(m){
        const isBot = m.role === 'assistant';
        const bg = isBot ? '#f5f3ff' : '#111';
        const color = isBot ? '#333' : '#fff';
        const align = isBot ? 'flex-start' : 'flex-end';
        const label = isBot ? 'Asystent AI' : 'Rozmówca';
        return '<div style="display:flex;justify-content:' + align + ';margin-bottom:12px;">' +
          '<div style="max-width:80%;background:' + bg + ';color:' + color + ';padding:10px 14px;border-radius:14px;font-size:13.5px;line-height:1.4;">' +
          '<div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.04em;opacity:0.6;margin-bottom:3px;">' + label + '</div>' +
          m.content +
          '</div></div>';
      }).join('');
  ...
  // Nieskonczone przewijanie - gdy dojezdzasz do konca listy, doladowujemy kolejne 20 rozmow.
  // Uzywamy "kursora" (data ostatniej zaladowanej rozmowy), nie numeru offsetu -
  // dzieki temu szybkosc NIE spada nawet przy tysiacach rozmow w bazie.
  let lastCreatedAt = ALL_CALLS.length > 0 ? ALL_CALLS[ALL_CALLS.length - 1].created_at_raw : null;
  let callsHasMore = ALL_CALLS.length === 20;
  let callsLoading = false;

  function renderCallRow(call, index){
    const row = document.createElement('tr');
    row.className = 'datarow';
    row.dataset.type = 'incoming';
    row.dataset.status = 'ok';
    row.dataset.callIndex = index;
    row.style.cursor = 'pointer';
    row.onclick = function(){ showTranscript(index); };
    row.innerHTML =
      '<td>' + call.data + '</td>' +
  ...
  async function loadMoreCalls(){
    if (callsLoading || !callsHasMore) return;
    callsLoading = true;
    const loadingEl = document.getElementById('callsLoading');
    if (loadingEl) loadingEl.style.display = 'block';

    try {
      const url = lastCreatedAt ? '/api/more-calls?after=' + encodeURIComponent(lastCreatedAt) : '/api/more-calls';
      ...
      newCalls.forEach(function(call){
        const index = ALL_CALLS.length;
        ALL_CALLS.push(call);
        table.appendChild(renderCallRow(call, index));
      });

      if (newCalls.length > 0) {
        lastCreatedAt = newCalls[newCalls.length - 1].created_at_raw;
      }
      callsHasMore = !!result.has_more;
    } catch (e) {
      callsHasMore = false;
    }
  ...
  const sentinel = document.getElementById('callsSentinel');
  if (sentinel && 'IntersectionObserver' in window) {
    const observer = new IntersectionObserver(function(entries){
      if (entries[0].isIntersecting) loadMoreCalls();
    }, { rootMargin: '200px' });
    observer.observe(sentinel);
  }

  // Przycisk "Wroc na gore" - pokazuje sie dopiero po przewinieciu kawalka w dol
  ...
  // Wyszukiwarka - przy pierwszym wpisaniu tekstu pobiera WSZYSTKIE rozmowy naraz (raz),
  // zeby wyszukiwanie dzialalo tez na rozmowach jeszcze nie doladowanych przez scroll.

  window.handleCallsSearch = async function(query){
    const q = query.trim().toLowerCase();

    if (!q) {
      // Wyczyszczono wyszukiwanie - pokaz z powrotem normalny widok
      document.querySelectorAll('#callsTable .datarow').forEach(row => row.style.display = '');
      if (sentinel) sentinel.style.display = '';
      updateCallsCounter();
      return;
    }

    if (!SEARCH_CACHE && !searchFetching) {
      searchFetching = true;
      const loadingEl = document.getElementById('callsLoading');
      if (loadingEl) { loadingEl.style.display = 'block'; loadingEl.textContent = 'Szukam...'; }
      try {
        const resp = await fetch('/api/search-calls');
        const result = await resp.json();
        SEARCH_CACHE = result.calls || [];
        searchTruncated = !!result.truncated;
        searchTotal = result.total || 0;

        // Dolacz do tabeli wiersze, ktorych jeszcze nie bylo (spoza pierwszych 20 zaladowanych)
        const table = document.getElementById('callsTable');
        for (let i = ALL_CALLS.length; i < SEARCH_CACHE.length; i++) {
          ALL_CALLS.push(SEARCH_CACHE[i]);
          const row = renderCallRow(SEARCH_CACHE[i], i);
    ...
    // Filtruj widoczne wiersze po numerze telefonu (kolumna "Z kim")
    document.querySelectorAll('#callsTable .datarow').forEach(function(row){
      const index = parseInt(row.dataset.callIndex, 10);
      const call = ALL_CALLS[index];
      const match = call && call.z_kim && call.z_kim.toLowerCase().includes(q);
...
def rozmowy():
    if not session.get("logged_in"):
        return redirect(url_for("login"))
    uid = session.get("uid", "")
    real_calls = get_user_calls(uid, limit=20)
    total_calls = get_user_calls_count(uid)
    return render_template_string(
        CALLS_PAGE,
        calls=real_calls,
...
@app.route("/api/more-calls")
def api_more_calls():
    if not session.get("logged_in"):
        return {"error": "not logged in"}, 401
    after = request.args.get("after") or None
    more_calls = get_user_calls(session.get("uid", ""), limit=20, after_created_at=after)
    return {"calls": more_calls, "has_more": len(more_calls) == 20}

@app.route("/api/search-calls")
def api_search_calls():
    """Pobiera duza partie rozmow uzytkownika naraz - uzywane tylko podczas wyszukiwania."""
    if not session.get("logged_in"):
        return {"error": "not logged in"}, 401
    uid = session.get("uid", "")
    search_limit = 2000
    all_calls = get_user_calls(uid, limit=search_limit)
    total = get_user_calls_count(uid)
    # Jesli rozmow jest wiecej niz przeszukalismy, ostrzegamy ze wyniki moga byc niepelne
    truncated = total > search_limit
    return {"calls": all_calls, "truncated": truncated, "total": total}
...
def api_test_chat():
    """
    Pozwala przetestowac logike AI (ta sama co w prawdziwej rozmowie telefonicznej)
    bez potrzeby dzwonienia - przydatne zanim Twilio jest oplacone.
    Uzywa TEGO SAMEGO profilu i instrukcji co prawdziwy bot na telefonie.
    """
    if not session.get("logged_in"):
        return {"error": "not logged in"}, 401
    ...
    if not message:
        return {"error": "Brak wiadomosci"}, 400

    uid = session.get("uid", "")
    user_data = get_user_doc_by_uid(uid) or {}
    system_prompt = build_profile_prompt(user_data)
    ...
    is_sufler = reply.strip().startswith("POTRZEBUJE_POMOCY:")

    return {"reply": reply, "is_sufler": is_sufler}
...
def outbound_twiml():
    ...
    # Jesli uzytkownik wybral konkretny jezyk dla TEGO zlecenia, uzywamy go -
    # w przeciwnym razie wracamy do domyslnego jezyka konta.
    bot_lang = BOT_LANGUAGES.get(requested_lang) or get_bot_language(owner_data)
    ...
    greeting = bot_lang["outbound_greeting_prefix"] + task
    CALL_CONTEXT[call_sid]["messages"].append({"role": "assistant", "content": greeting})
...
def start_outbound_call():
    ...
    if requested_lang not in BOT_LANGUAGES:
        requested_lang = ""
    ...
    twiml_url = (
        ...
        "&lang=" + requests.utils.quote(requested_lang)
    )
    status_callback_url = request.url_root + "call-status"
