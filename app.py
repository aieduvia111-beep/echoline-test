...
<script type="module">
    ...
    if (ready) {
      ...
      voicesList.innerHTML = `
        <div class="voice-item">
          <div class="play-btn"><svg viewBox="0 0 24 24"><polygon points="6 3 20 12 6 21 6 3"/></svg></div>
          <div style="flex:1;">
            <div class="voice-item-name">Twój głos <span class="voice-tag">Aktywny</span></div>
            <div class="voice-item-date">Głos sklonowany</div>
          </div>
        </div>`;
    } else {
  ...
  async function uploadRecording(){
    ...
    if (result.voice_id && currentUid) {
      ...
      document.getElementById("statusSub").textContent = result.error || "Nieznany błąd";
